...
[Main]
[Main.General]
Account = { Login = ${MINECRAFT_USERNAME}, Password = ${MINECRAFT_PASSWORD} } # Login=Email or Name. Use "-" as password for offline mode. Leave blank to prompt user on startup.
Server = { Host = ${MINECRAFT_SERVER_IP} }         # The address of the game server, "Host" can be filled in with domain name or IP address. (The "Port" field can be deleted, it will be resolved automatically)
...
Retries = 10                                # Retries when failing to relog to the server. use -1 for unlimited retries.
